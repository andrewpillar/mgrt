...
func catCmd(cmd *Command, args []string) {
	...
	if len(args) < 1 {
		fmt.Fprintf(os.Stderr, "usage: %s <revisions,...>\n", cmd.Argv0)
		os.Exit(1)
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			fmt.Fprintf(os.Stderr, "%s: no migrations created\n", cmd.Argv0)
			os.Exit(1)
		}
		fmt.Fprintf(os.Stderr, "%s: failed to cat revision(s): %s\n", cmd.Argv0, err)
		os.Exit(1)
	}
	...
	for _, id := range args {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s: failed to cat revision: %s\n", cmd.Argv0, err)
			os.Exit(1)
		}
